...
# ============================================================
# CDI HISTÓRICO – Banco Central (série 12)
# ============================================================
...
def backtest_ap(precos, dividendos, cdi_df, prazo_du, perda_max, preco_put_cotada):
    ...
    # Spread
    markup = preco_put_cotada / preco_put_justa_hoje if preco_put_justa_hoje > 0 else 1.0
    ...
    for i in range(len(p0)):
        ...
        preco_put_adj = preco_put_just * markup
...
st.title("📈 Backtest – Collar & Alocação Protegida (com CDI e Spread da PUT)")
st.markdown("Comparação real com CDI (BACEN série 12), volatilidade dinâmica e PUT ajustada por spread do operador.")

# Carregar CDI
try:
    cdi_df = carregar_cdi_com_fator()
except Exception:
    st.error("Erro ao carregar CDI (série 12).")
    st.stop()
...
with tab_c:
    ...
    ticker_c = st.text_input("Ticker:", "EZTC3.SA", key="t1")
    prazo_du_c = st.number_input("Prazo (dias úteis)", 10, 252, 63, key="p1")
    ganho_c = st.number_input("Ganho Máx (%)", 0.0, 50.0, 8.0, key="g1") / 100
    perda_c = st.number_input("Perda Máx (%)", 0.0, 50.0, 8.0, key="l1") / 100
    ...
    if st.button("Rodar Collar"):
        ...
        if resultado:
            df, resumo, _ = resultado

            c1, c2 = st.columns(2)
            c1.metric("Estrutura Favorável (%)", f"{100*resumo['pct_deu_certo']:.1f}%")
            c2.metric("Bateu CDI (%)", f"{100*resumo['pct_bate_cdi']:.1f}%")
...
with tab_ap:
    ...
    ticker_ap = st.text_input("Ticker:", "EZTC3.SA", key="t2")
    prazo_du_ap = st.number_input("Prazo (dias úteis)", 10, 252, 63, key="p2")
    perda_ap = st.number_input("Perda máxima protegida (%)", 0.0, 50.0, 5.0, key="l2") / 100
    preco_put_cotada = st.number_input("Preço cotado da PUT hoje (R$):", 0.01, 50.0, 0.50, key="put")
    ...
    if st.button("Rodar AP"):
        ...
        if resultado:
            ...
            c1.metric("PUT justa hoje (BSL)", f"R$ {resumo['preco_put_justa_hoje']:.4f}")
            c2.metric("PUT cotada", f"R$ {resumo['preco_put_cotada']:.4f}")
            c3.metric("Spread aplicado", f"{(resumo['markup']-1)*100:.1f}%")

            c4, c5 = st.columns(2)
            c4.metric("Estrutura Favorável (%)", f"{100*resumo['pct_deu_certo']:.1f}%")
            c5.metric("Bateu CDI (%)", f"{100*resumo['pct_bate_cdi']:.1f}%")
